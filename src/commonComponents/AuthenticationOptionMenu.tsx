...
import { AuthOptionProps, MenuOptionProps } from "../interfaces/AuthOptionInterfaces";

export default function AuthenticationOptionMenu(props: AuthOptionProps) {
    const [optionSelected, setOptionSelected] = useState(props.authIndex);

    const selectOption = (newOptionSelected: number): void => {
        props.dispatch({
    ...
    return (
        <div id="auth_option_menu" className="flex-block column">
            ...
            <Button id="select-auth-method" onClick={() => props.dispatch({ type: FlowDispatchTypes.NEXT })}>
