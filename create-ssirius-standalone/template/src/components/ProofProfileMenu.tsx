    ...
    useEffect(() => {
        const getProofOptions = async () => {
            try {
                ...
                const errorMsg: string = e.hasOwnProperty('message') ? e.message : 'An unknown error occurred when collecting the proof profile options.'
                setProofsOptionsError(errorMsg);
            } finally {
                ...
                <FormControl className="form-control">
                    ...
                    <Select
                        ...
                        {proofOptions.map((option, index) => {
                            return (
                                <MenuItem value={index} key={index}>{`${index + 1
